...
public class HomeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			ArrayList<FollowerVO> followerList = impl.followerList(mseq, conn);
			ArrayList<ContentVO> contentList = new ArrayList<>();
			if(followerList.size()>0){
				for(int i=0; i<followerList.size();i++){
					int requesterSeq = followerList.get(i).getRequesterSeq();
					ArrayList<ContentVO> tempList = impl.selectMemberSeqList(requesterSeq, conn);
					if(tempList.size()>0){
						contentList.addAll(tempList);
						for(int r=0; r<contentList.size();r++){
							ArrayList<ReplyVO> rlist = new ArrayList<ReplyVO>();
							rlist = impl.replyList(contentList.get(r).getCseq(), conn);
							contentList.get(r).setRlist(rlist);
						}
