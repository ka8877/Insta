               ...
               <div class="row">
                  <article id="01" class="page-block col-md-12 page hentry">
                     ...
                     <div class="page-detail text-center">
                        <h2 class="entry-title page-title">${name }<a href="#!"></a></h2>
                        <div class="metas" id="kang">
                           게시물: <a class="meta-date">${contentCnt }</a>/&nbsp;
                           팔로워: <a href="#!" class="meta-author">${followerCnt }</a>/&nbsp;
                           팔로우: <a href="#!" class="meta-comment">${requesterCnt }</a><br>
							  <c:if test="${followGubun == 'true'}" > 
                             <p>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-blue"  
                             onclick="location='/followInsert?followGubun=${followGubun}&requesterSeq=${requesterSeq}'" >팔로잉</button></p>
                              </c:if>
                              	<c:if test="${followGubun == 'false'}" > 
                             <p>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-rose"  
                             onclick="location.href='/followInsert?followGubun=${followGubun}&requesterSeq=${requesterSeq}'" >팔로우</button></p>
                              </c:if>
                        </div>
                     </div>
...
<!-- <script>
	function insertFollow(gubun, id) {
		$.ajax({
			type: 'GET',
			url: "/followInsert.ajax",
			data: {
				requesterId:id,
				followGubun:gubun
			},
			success: function(resultData){
				$("#kang").empty();
				var noticeHtml = '';
				$.each(resultData, function(index, notice){
					if(follow)
				}
			}
		})
