...
public class PagingUtil {
	...
	public PagingUtil(String url, int currentPage, int totRecord, int blockCount, int blockPage) {
		...
		pagingHtml.append("<div class='col-md-6'><div class='pagination'><ul class='pagination'>");
		if (currentPage > blockPage) {
			pagingHtml.append("<li class='next'><a href='"+ url +"&currentPage="  + (startBlock - 1) + "'>");
...
//		pagingHtml.append("  |  ");
		...
		if (maxPage - startBlock >= blockPage) {
			pagingHtml.append("<li class='next'><a href='" + url +"&currentPage="  + (endBlock + 1) + "'>");
			pagingHtml.append("다음");
			pagingHtml.append("</a></li>");
		}
		pagingHtml.append("</ul></div></div>");
